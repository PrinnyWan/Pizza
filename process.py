class processdata:
    ...
    def run(self, inputdata):
        ...
        self.C = inputdata.C
        self.maxarea = 0
        self.runhelper(0, 0, 0, [], R, C, L, H, visited, mushrooms_map, tomato_map)
    ...
    def runhelper(self, curx, cury, curarea, stack, R, C, L, H, visited, map1, map2):
        ...
        for slice_x in range(H + 1):
            for slice_y in range(H + 1):
                if curx + slice_x < R and cury + slice_y < C and 2*L <= (slice_x + 1) * (slice_y + 1) <= 2*H and self.valid(curx, cury, slice_x, slice_y, L, H, map1, map2, visited):
                    ...
                    self.runhelper(nx, ny, curarea + (slice_x + 1) * (slice_y + 1), stack, R, C, L, H, visited, map1, map2)
                    self.changevisited(curx, cury, slice_x, slice_y, visited, 0)
                    stack.pop()
        visited[self.getid(curx, cury)] = 1
        nx, ny = self.findnextpoint(curx, cury, R, C, visited)
        self.runhelper(nx, ny, curarea, stack, R, C, L, H, visited, map1, map2)
